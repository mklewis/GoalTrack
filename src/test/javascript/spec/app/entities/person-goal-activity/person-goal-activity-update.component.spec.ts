/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import Router from 'vue-router';

import dayjs from 'dayjs';
import { DATE_TIME_LONG_FORMAT } from '@/shared/date/filters';

import * as config from '@/shared/config/config';
import PersonGoalActivityUpdateComponent from '@/entities/person-goal-activity/person-goal-activity-update.vue';
import PersonGoalActivityClass from '@/entities/person-goal-activity/person-goal-activity-update.component';
import PersonGoalActivityService from '@/entities/person-goal-activity/person-goal-activity.service';

import GoalService from '@/entities/goal/goal.service';

import PersonService from '@/entities/person/person.service';

const localVue = createLocalVue();

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
const router = new Router();
localVue.use(Router);
localVue.component('font-awesome-icon', {});
localVue.component('b-input-group', {});
localVue.component('b-input-group-prepend', {});
localVue.component('b-form-datepicker', {});
localVue.component('b-form-input', {});

describe('Component Tests', () => {
  describe('PersonGoalActivity Management Update Component', () => {
    let wrapper: Wrapper<PersonGoalActivityClass>;
    let comp: PersonGoalActivityClass;
    let personGoalActivityServiceStub: SinonStubbedInstance<PersonGoalActivityService>;

    beforeEach(() => {
      personGoalActivityServiceStub = sinon.createStubInstance<PersonGoalActivityService>(PersonGoalActivityService);

      wrapper = shallowMount<PersonGoalActivityClass>(PersonGoalActivityUpdateComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: {
          personGoalActivityService: () => personGoalActivityServiceStub,

          goalService: () => new GoalService(),

          personService: () => new PersonService(),
        },
      });
      comp = wrapper.vm;
    });

    describe('load', () => {
      it('Should convert date from string', () => {
        // GIVEN
        const date = new Date('2019-10-15T11:42:02Z');

        // WHEN
        const convertedDate = comp.convertDateTimeFromServer(date);

        // THEN
        expect(convertedDate).toEqual(dayjs(date).format(DATE_TIME_LONG_FORMAT));
      });

      it('Should not convert date if date is not present', () => {
        expect(comp.convertDateTimeFromServer(null)).toBeNull();
      });
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', async () => {
        // GIVEN
        const entity = { id: 'ABC' };
        comp.personGoalActivity = entity;
        personGoalActivityServiceStub.update.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(personGoalActivityServiceStub.update.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', async () => {
        // GIVEN
        const entity = {};
        comp.personGoalActivity = entity;
        personGoalActivityServiceStub.create.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(personGoalActivityServiceStub.create.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });
    });

    describe('Before route enter', () => {
      it('Should retrieve data', async () => {
        // GIVEN
        const foundPersonGoalActivity = { id: 'ABC' };
        personGoalActivityServiceStub.find.resolves(foundPersonGoalActivity);
        personGoalActivityServiceStub.retrieve.resolves([foundPersonGoalActivity]);

        // WHEN
        comp.beforeRouteEnter({ params: { personGoalActivityId: 'ABC' } }, null, cb => cb(comp));
        await comp.$nextTick();

        // THEN
        expect(comp.personGoalActivity).toBe(foundPersonGoalActivity);
      });
    });

    describe('Previous state', () => {
      it('Should go previous state', async () => {
        comp.previousState();
        await comp.$nextTick();

        expect(comp.$router.currentRoute.fullPath).toContain('/');
      });
    });
  });
});
